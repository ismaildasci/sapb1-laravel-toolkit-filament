...
class InvoiceResource extends Resource
{
    ...
    {
        return ['DocNum', 'CardCode', 'CardName'];
    }
    ...
    {
        $balance = (float) ($record->DocTotal ?? 0) - (float) ($record->PaidToDate ?? 0);

        /** @phpstan-ignore argument.type */
        return [
            __('sapb1-filament::resources.invoice.fields.card_name') => (string) ($record->CardName ?? ''),
            __('sapb1-filament::resources.invoice.fields.doc_total') => number_format((float) ($record->DocTotal ?? 0), 2).' TRY',
            __('sapb1-filament::resources.invoice.fields.balance') => number_format($balance, 2).' TRY',
        ];
    }
