...
class InvoicesRelationManager extends SapRelationManager
{
    protected static ?string $title = null;

    public static function getTitle(Model $ownerRecord, string $pageClass): string
    {
        return static::$title ?? __('sapb1-filament::resources.relation_managers.invoices.title');
    }
    ...
    protected function getRecords(
        ...
        $query = Invoice::where('CardCode', $cardCode);
        ...
        $mapped = collect($records->map(fn (SapB1Model $invoice): array => array_merge(
            $invoice->toArray(),
            ['__key' => (string) $invoice->getKey()],
        )));
        ...
        return $table
            ->columns([
                Tables\Columns\TextColumn::make('DocNum')
                    ->label(__('sapb1-filament::resources.relation_managers.invoices.columns.doc_num'))
                    ->weight('bold')
                    ->url(fn (array $record): string => InvoiceResource::getUrl('view', ['record' => $record['__key']]))
                    ->sortable(),

                Tables\Columns\TextColumn::make('CardName')
                    ->label(__('sapb1-filament::resources.relation_managers.invoices.columns.card_name'))
                    ->wrap()
                    ->sortable(),

                Tables\Columns\TextColumn::make('DocDate')
                    ->label(__('sapb1-filament::resources.relation_managers.invoices.columns.doc_date'))
                    ->date()
                    ->sortable(),

                Tables\Columns\TextColumn::make('DocTotal')
                    ->label(__('sapb1-filament::resources.relation_managers.invoices.columns.doc_total'))
                    ->money('TRY')
                    ->alignEnd()
                    ->sortable(),

                Tables\Columns\TextColumn::make('PaidToDate')
                    ->label(__('sapb1-filament::resources.relation_managers.invoices.columns.paid_to_date'))
                    ->money('TRY')
                    ->alignEnd()
                    ->sortable(),

                Tables\Columns\TextColumn::make('DocumentStatus')
                    ->label(__('sapb1-filament::resources.relation_managers.invoices.columns.status'))
                    ->badge()
